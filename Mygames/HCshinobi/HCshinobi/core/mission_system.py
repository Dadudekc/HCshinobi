...
class MissionSystem:
    ...
    def get_completed_missions(self, user_id: int) -> List[str]:
        """Get list of completed mission IDs for a user."""
        return self.completed_missions.get(str(user_id), [])

    def get_available_missions(self, character_or_level: Union[Character, int]) -> List[Dict[str, Any]]:
        """Get available missions for a character or level.
        
        Args:
            character_or_level: Either a Character object or an integer level
            
        Returns:
            List of available mission dictionaries
        """
        if isinstance(character_or_level, Character):
            character = character_or_level
            level = character.level
            rank = character.rank
        else:
            level = character_or_level
            rank = None
            
        available = []
        for mid, mission in self.available_missions.items():
            reqs = mission.requirements
            if reqs.get("level", 1) <= level:
                if rank is None or reqs.get("rank", "D") <= rank:
