...
async def test_devlog_commands(devlog_commands_cog, mock_bot, command_name, params):
    ...
    if command_name == "devlog":
        trace.assert_interaction_sequence(
            ...
            ("followup_send", {"content": None})  # followup_send
        )
...
@pytest.mark.asyncio
async def test_devlog_commands_no_permission(devlog_commands_cog):
    """Test that non-administrators cannot use admin commands."""
    trace = InteractionTrace()
    mock_ctx = trace.create_mock_ctx()
    mock_ctx.user.guild_permissions.administrator = False
    ...
    trace.assert_interaction_sequence(
        ("response_send", {"content": "❌ You don't have permission to use this command.", "ephemeral": True})
    )
